...
TEST_F(LimitOrderTest, TestDeterminism_TwoBooksInterleavedMatchStandaloneRuns) {
    struct Action {
        enum class Kind : uint8_t { Add, Cancel, SetMatching };
        Kind kind;
        OrderID id = 0;
    ...
    const auto applyAction = [](const Action& action, const std::shared_ptr<OrderBook<Notification>>& localOb) {
        if (action.kind == Action::Kind::SetMatching) {
            localOb->setMatching(action.matching);
        } else if (action.kind == Action::Kind::Cancel) {
            localOb->cancelOrder(action.id);
    ...
    const std::vector<Action> actionsA = {
        {Action::Kind::Add, 1, Type::Limit, Side::Buy, Decimal(0, 0), Decimal(100, 0), Flag::None},
        {Action::Kind::Add, 2, Type::Limit, Side::Buy, Decimal(2, 0), Decimal(90, 0), Flag::None},
        {Action::Kind::Cancel, 2},
        {Action::Kind::SetMatching, 0, Type::Limit, Side::Buy, Decimal(0, 0), Decimal(0, 0), Flag::None, false},
        ...
        {Action::Kind::Add, 4, Type::Limit, Side::Sell, Decimal(1, 0), Decimal(80, 0), Flag::None},
        {Action::Kind::SetMatching, 0, Type::Limit, Side::Buy, Decimal(0, 0), Decimal(0, 0), Flag::None, true},
        {Action::Kind::Add, 5, Type::Limit, Side::Buy, Decimal(3, 0), Decimal(100, 0), Flag::None},
        {Action::Kind::Add, 6, Type::Limit, Side::Sell, Decimal(5, 0), Decimal(90, 0), Flag::IoC},
        {Action::Kind::Add, 7, Type::Limit, Side::Buy, Decimal(4, 0), Decimal(120, 0), Flag::FoK},
        {Action::Kind::Add, 8, Type::Market, Side::Sell, Decimal(3, 0), Decimal(0, 0), Flag::AoN},
        {Action::Kind::Cancel, 999},
    };

    const std::vector<Action> actionsB = {
        {Action::Kind::Add, 101, Type::Limit, Side::Sell, Decimal(0, 0), Decimal(100, 0), Flag::None},
        {Action::Kind::Add, 102, Type::Limit, Side::Sell, Decimal(2, 0), Decimal(110, 0), Flag::None},
        {Action::Kind::Cancel, 102},
        {Action::Kind::SetMatching, 0, Type::Limit, Side::Buy, Decimal(0, 0), Decimal(0, 0), Flag::None, false},
        {Action::Kind::Add, 103, Type::Market, Side::Sell, Decimal(1, 0), Decimal(0, 0), Flag::None},
        {Action::Kind::Add, 104, Type::Limit, Side::Buy, Decimal(1, 0), Decimal(120, 0), Flag::None},
        {Action::Kind::SetMatching, 0, Type::Limit, Side::Buy, Decimal(0, 0), Decimal(0, 0), Flag::None, true},
        {Action::Kind::Add, 105, Type::Limit, Side::Sell, Decimal(3, 0), Decimal(100, 0), Flag::None},
        {Action::Kind::Add, 106, Type::Limit, Side::Buy, Decimal(5, 0), Decimal(110, 0), Flag::IoC},
        {Action::Kind::Add, 107, Type::Limit, Side::Sell, Decimal(4, 0), Decimal(80, 0), Flag::FoK},
        {Action::Kind::Add, 108, Type::Market, Side::Buy, Decimal(3, 0), Decimal(0, 0), Flag::AoN},
        {Action::Kind::Cancel, 1999},
    };

    const auto runSequence = [&applyAction](const std::vector<Action>& actions) {
        Notification notification;
        auto localOb = std::make_shared<orderbook::OrderBook<Notification>>(notification);
        for (const auto& action : actions) {
    ...
    const auto [standaloneReportsA, standaloneBookA, standaloneLastPriceA] = runSequence(actionsA);
    const auto [standaloneReportsB, standaloneBookB, standaloneLastPriceB] = runSequence(actionsB);

    Notification interleavedN1;
    Notification interleavedN2;
    auto interleavedOb1 = std::make_shared<orderbook::OrderBook<Notification>>(interleavedN1);
    auto interleavedOb2 = std::make_shared<orderbook::OrderBook<Notification>>(interleavedN2);

    const size_t maxActions = std::max(actionsA.size(), actionsB.size());
    for (size_t i = 0; i < maxActions; ++i) {
        if (i < actionsA.size()) {
            applyAction(actionsA[i], interleavedOb1);
        }
        if (i < actionsB.size()) {
            applyAction(actionsB[i], interleavedOb2);
        }
    }

    ASSERT_EQ(interleavedN1.Strings(), standaloneReportsA);
    ASSERT_EQ(interleavedN2.Strings(), standaloneReportsB);
    ASSERT_EQ(interleavedOb1->toString(), standaloneBookA);
    ASSERT_EQ(interleavedOb2->toString(), standaloneBookB);
    ASSERT_EQ(interleavedOb1->last_price, standaloneLastPriceA);
    ASSERT_EQ(interleavedOb2->last_price, standaloneLastPriceB);
}

TEST_F(LimitOrderTest, TestDeterminism_RecoverAtEveryMidpointThenReplaySuffix) {
    struct Action {
        enum class Kind : uint8_t { Add, Cancel, SetMatching };
        Kind kind;
        OrderID id = 0;
