...
class LimitOrderTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        tok = 0;
        n = Notification();
        ob = std::make_shared<orderbook::OrderBook>(n);
    }

    void TearDown() override {}

    void processLine(std::shared_ptr<OrderBook> ob, const std::string& line) {
    ...
    void processOrders(std::shared_ptr<OrderBook> ob, const std::string& input, int prefix) {
    ...
    void addDepth(std::shared_ptr<OrderBook> ob, int prefix = 0) {
